...
public class CommandConfirmRequestConsumer {
    ...
    public CommandConfirmRequestConsumer(
    ...
    public void onMessage(String payload) {
        CommandConfirmRequestEvent event = parse(payload);
    ...
    private void processWithRetry(CommandConfirmRequestEvent event, String rawPayload, String processKey) {
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                recordFailureAndCompensate(kafkaProperties.getCommandConfirmRequestTopic(), rawPayload, failure);
                throw wrapForDlq(kafkaProperties.getDlqTopicSuffix(), failure);
            }
        }
    }

    private void processOnce(CommandConfirmRequestEvent event, String processKey) {
        CommandPipelineOutcome outcome = pipelineService.fromCommandConfirm(event);
    ...
    private CommandConfirmRequestEvent parse(String payload) {
        try {
    ...
    private String dedupeKey(CommandConfirmRequestEvent event) {
        if (event.idempotencyKey() != null && !event.idempotencyKey().isBlank()) {
            return event.idempotencyKey();
        }
        CommandConfirmRequestPayload payload = event.payload();
        ...
        return executionId + ":confirm:" + confirmRound + ":" + event.seq();
    }
