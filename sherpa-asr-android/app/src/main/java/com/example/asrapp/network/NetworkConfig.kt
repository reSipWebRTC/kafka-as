...
data class NetworkConfig(
    ...
    val accessToken: String = DEFAULT_ACCESS_TOKEN,
    val ttsReadyFallbackDelayMs: Long = DEFAULT_TTS_READY_FALLBACK_DELAY_MS
) {
    companion object {
        const val DEFAULT_SERVER_URL = "http://127.0.0.1:8080"
        const val DEFAULT_TIMEOUT = 3000L
        const val DEFAULT_RETRY_COUNT = 2
        const val DEFAULT_TENANT_ID = "tenant-home"
        const val DEFAULT_USER_ID = "android-user-1"
        const val DEFAULT_SOURCE_LANG = "zh-CN"
        const val DEFAULT_TARGET_LANG = "zh-CN"
        const val DEFAULT_ACCESS_TOKEN = ""
        const val DEFAULT_TTS_READY_FALLBACK_DELAY_MS = 1500L
    }
    ...
    /**
     * 获取网关 WebSocket URL（默认 /ws/audio）
     */
    fun getGatewayWsEndpoint(): String {
        val normalized = serverUrl.trim().removeSuffix("/")
        if (normalized.isBlank()) {
            return "ws://127.0.0.1:8080/ws/audio"
        }

        if (normalized.startsWith("ws://") || normalized.startsWith("wss://")) {
            return if (normalized.endsWith("/ws/audio")) normalized else "$normalized/ws/audio"
        }
        ...
        return if (wsBase.endsWith("/ws/audio")) wsBase else "$wsBase/ws/audio"
    }
    ...
    fun isValid(): Boolean {
        return serverUrl.isNotBlank() &&
               ...
               tenantId.isNotBlank() &&
               userId.isNotBlank() &&
               sourceLang.isNotBlank() &&
               targetLang.isNotBlank() &&
               timeoutMs > 0
    }
}
