...
object ModelWarmupManager {
    ...
    private suspend fun waitForAssetsReady(context: Context) {
        ...
        while (System.currentTimeMillis() - startWait < maxWait) {
            val asrReady = File(context.filesDir, "asr/encoder.int8.onnx").exists() &&
                File(context.filesDir, "asr/decoder.int8.onnx").exists() &&
                File(context.filesDir, "asr/tokens.txt").exists()
            val vadReady = File(context.filesDir, "vad/silero_vad.onnx").exists()
            val ttsReady = File(context.filesDir, "tts").exists()
            if (asrReady && vadReady && ttsReady) {
                return
            }
            delay(100)
        }
    ...
    fun takeAsrModels(): Pair<OnlineRecognizer, Vad>? {
        return runBlocking {
            mutex.withLock {
                if (!_asrWarmed) return@withLock null
                val recognizer = cachedRecognizer ?: return@withLock null
                val vad = cachedVad ?: return@withLock null
                cachedRecognizer = null
                cachedVad = null
                // 标记为未预热，因为模型已被取走
                _asrWarmed = false
    ...
    fun takeTtsModel(): OfflineTts? {
        return runBlocking {
            mutex.withLock {
                if (!_ttsWarmed) return@withLock null
                val tts = cachedTts ?: return@withLock null
                cachedTts = null
                // 标记为未预热，因为模型已被取走
                _ttsWarmed = false
                tts
