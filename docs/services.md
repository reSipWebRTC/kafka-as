...
| `translation-worker` | 已落地骨架 | 两段式消费与发布：`asr.final -> translation.request -> translation.result`，并按租户 `sessionMode` 分流（仅 `TRANSLATION` 处理，`SMART_HOME` 忽略）；默认 placeholder + 可切换 HTTP/OpenAI 翻译适配，OpenAI 适配已具备 health 探测、并发保护、错误语义映射与引擎级指标 | Kafka |
...
- `asr.final` 消费按租户 `sessionMode` 分流（仅 `TRANSLATION` 处理，`SMART_HOME` 忽略）
