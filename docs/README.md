...
- `command-worker` 消费 `asr.final`（`SMART_HOME` 会话）与命令回执事件，并发布 `command.dispatch` / `command.result`
- `tts-orchestrator` 消费 `translation.result` 与 `command.result` 并发布 `tts.request` / `tts.chunk` / `tts.ready`
