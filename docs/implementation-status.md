...
- `translation-worker` 已补齐两段式翻译治理链路：`asr.final -> translation.request -> translation.result`，并按租户 `sessionMode=TRANSLATION` 门控（`SMART_HOME` 场景忽略）（含按租户策略驱动重试/DLQ、幂等与补偿）
...
| `translation-worker` | `8083` | 消费 `asr.final` 后发布 `translation.request`，再消费 `translation.request` 生成并发布 `translation.result`；按租户 `sessionMode` 路由（仅 `TRANSLATION` 生效，`SMART_HOME` 忽略）；默认 placeholder 翻译 + 可切换 HTTP/OpenAI 翻译适配（含 OpenAI v2 响应兼容、health 探测、并发保护、错误语义映射）；按租户策略驱动重试/DLQ（含控制面失败回退） | OpenAI 真机容量/故障演练、术语治理、上下文增强 |
