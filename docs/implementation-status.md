...
- `tools/mock-iam-rbac-drill.sh` 已补齐无真实 IAM 环境下的 simulated/mock 演练入口（JWKS/JWT 链路、claim 矩阵、失败演练、告警规则校验）
