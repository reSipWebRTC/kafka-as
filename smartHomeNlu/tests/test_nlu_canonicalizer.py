...
def test_canonicalize_preserves_automation_sub_intent() -> None:
    raw = IntentJson(
        intent="SYSTEM",
...
def test_canonicalize_control_unknown_no_longer_defaults_to_power_on() -> None:
    raw = IntentJson(
        intent="CONTROL",
        sub_intent="control",
    ...
    assert out.sub_intent == "unknown"
    assert out.intent == "CHITCHAT"


def test_canonicalize_system_unknown_does_not_default_to_backup() -> None:
    raw = IntentJson(
        intent="SYSTEM",
        sub_intent="system_task",
        slots={},
        confidence=0.8,
    )
