...
def test_mcp_url_normalization_adds_default_mcp_path() -> None:
    ...
    adapter_without_scheme = HaMcpAdapter(mcp_url="127.0.0.1:8086")
    assert adapter_without_scheme._mcp_url == "http://127.0.0.1:8086/mcp"

    adapter_without_scheme_with_path = HaMcpAdapter(mcp_url="127.0.0.1:8086/private_demo")
    assert adapter_without_scheme_with_path._mcp_url == "http://127.0.0.1:8086/private_demo"
...
def test_invoke_remote_tool_flattens_exception_group_message() -> None:
    exception_group_cls = globals().get("ExceptionGroup")

    class FakeTaskGroupError(RuntimeError):
        def __init__(self) -> None:
            super().__init__("unhandled errors in a TaskGroup (1 sub-exception)")
            self.exceptions = [ConnectionError("connect ECONNREFUSED 127.0.0.1:8123")]

    def failing_runner(tool_name: str, params: Dict[str, Any]) -> Dict[str, Any]:
        if exception_group_cls is not None:
            raise exception_group_cls(
                "unhandled errors in a TaskGroup",
                [ConnectionError("connect ECONNREFUSED 127.0.0.1:8123")],
            )
        raise FakeTaskGroupError()

    adapter = HaMcpAdapter(
        mcp_url="http://fake-ha-mcp.local/mcp",
        remote_tool_runner=failing_runner,
    )

    result = adapter._invoke_remote_tool("ha_search_entities", {"query": "灯", "limit": 1})

    assert result["success"] is False
    assert result["error_code"] == "UPSTREAM_ERROR"
    error_text = str(result.get("error", "")).lower()
    assert "econnrefused" in error_text
    assert "taskgroup" in error_text or "unhandled errors" in error_text
