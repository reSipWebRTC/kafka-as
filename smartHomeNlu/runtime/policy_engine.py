...
class PolicyEngine:
    ...
    def _plan_tool(self, intent_json: Dict[str, Any]) -> str:
        ...
        if sub_intent == "automation_create":
            return "ha_create_automation"
        if sub_intent == "automation_cancel":
            return "ha_remove_automation"
    ...
    def _risk_level(self, intent_json: Dict[str, Any]) -> str:
        ...
        if sub_intent == "unlock":
            return "critical"
        if sub_intent in {"automation_create", "automation_cancel"}:
            return "medium"
        if intent == "SYSTEM":
            return "high"
        if intent in {"SCHEDULE", "AUTOMATION"}:
    ...
    def _make_idempotency_key(self, user_id: str, intent_json: Dict[str, Any]) -> str:
        ...
        key_data = {
            ...
            "entity_id": str(slots.get("entity_id", "")),
            "attribute": str(slots.get("attribute", "")),
            "value": str(slots.get("value", "")),
        }
        if str(intent_json.get("sub_intent", "")) in {"automation_create", "automation_cancel"}:
            key_data.update(
                {
                    "trigger_type": str(slots.get("trigger_type", "")),
                    "time_expression": str(slots.get("time_expression", "")),
                    "condition": str(slots.get("condition", "")),
                    "automation_query": str(slots.get("automation_query", "")),
                }
            )
