...
def _trigger_to_dict(trigger: Any) -> Dict[str, Any]:
    """将 TriggerSpec 转为普通字典并清理空值。"""
    if trigger is None:
        return {}
    if hasattr(trigger, "model_dump"):
        raw = trigger.model_dump(exclude_none=True)
    elif hasattr(trigger, "dict"):
        raw = trigger.dict(exclude_none=True)
...
def _extract_slots(cmd: SemanticCommand) -> Dict[str, Any]:
    ...
    if cmd.intent in ("automation_create", "automation_cancel"):
        action = _sv_normalized(cmd.action) or _sv_raw(cmd.action)
        if action:
            slots["action"] = action
        if cmd.trigger_type:
            slots["trigger_type"] = cmd.trigger_type
        trigger_payload = _trigger_to_dict(cmd.trigger)
        if trigger_payload:
            slots["trigger"] = trigger_payload
        time_expr = _sv_normalized(cmd.time) or _sv_raw(cmd.time)
        if not time_expr and isinstance(trigger_payload, dict):
            trigger_time = trigger_payload.get("time_expression")
            if trigger_time:
                time_expr = str(trigger_time)
        if time_expr:
            slots["time_expression"] = time_expr
        if cmd.condition:
            slots["condition"] = cmd.condition
        if cmd.raw_text:
            slots["automation_query"] = cmd.raw_text
...
def _decision_to_intent_json(decision: SemanticDecision) -> IntentJson:
    ...
    if decision.requires_confirmation:
        slots["_semantic_requires_confirmation"] = True
        if decision.confirmation_message:
            slots["_semantic_confirmation_message"] = decision.confirmation_message
