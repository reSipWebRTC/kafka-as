...
_SUB_INTENT_ALIASES = {
    ...
    "automation_cancel": "automation_cancel",
    "cancel_automation": "automation_cancel",
    "backup": "backup",
...
def _infer_sub_intent_by_frame(intent: str, sub_intent: str, slots: Dict[str, Any]) -> str:
    ...
    if str(intent).upper() == "SCENE":
        return "activate_scene"
    if any(token in raw for token in ("automation_create", "create_automation", "自动化")):
        if any(token in raw for token in ("cancel", "取消", "删除", "停用")):
            return "automation_cancel"
        return "automation_create"
    if str(intent).upper() == "SYSTEM":
        if any(token in raw for token in ("backup", "create_backup", "备份")):
            return "backup"
        return "unknown"
