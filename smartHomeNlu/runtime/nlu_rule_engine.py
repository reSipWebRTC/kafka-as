    ...
    def _collect_anchor_window_candidates(
        ...
        for anchor in anchors:
            ...
            if denoised_span and denoised_span != raw_span and len(denoised_span) >= 3:
                ...
                if not command:
                    continue
                if not self._is_viable_recovery_candidate(denoised_span, command):
                    continue
                if self._is_implausible_command_candidate(
                    ...
                    continue
                denoised_end = anchor + len(denoised_span)
                score = self._score_recovery_candidate(anchor, denoised_end, command) - 0.05
                candidates.append((anchor, denoised_end, command, score))
        ...
        if any(token in span for token in ("我的", "你的", "他的", "她的", "它的")):
            ...
            if device_category == "light":
                ...
                if command.device in {"灯", "灯光"}:
                    return False
        inferred_fragment = self._infer_recovery_device_fragment(span, command)
        canonical_action = self._canonicalize_action(command.action)
        is_power_command = canonical_action in {"打开", "关闭", "开", "关"}
        has_value_payload = bool(command.parameter or command.value or command.unit)
        if (
            is_power_command
            and not has_value_payload
            and not self._has_device_anchor_evidence(
                span,
                command,
                inferred_fragment=inferred_fragment,
            )
        ):
            return False
        if self._has_recovery_residue(
            span,
            command,
            extra_consumed_tokens=[inferred_fragment] if inferred_fragment else None,
        ):
            return False
        return True

    def _infer_recovery_device_fragment(self, span: str, command: Command) -> str:
        """恢复候选里推断设备时，回收原始文本中的设备残片。"""
        device = (command.device or "").strip()
        if not device:
            return ""
        if device in span:
            return ""

        action = self._extract_action(span)
        location = command.location or self._extract_location(span)
        parameter_kind = self._infer_parameter_kind(span, command.parameter or "")
        payload = self._extract_device_residue_payload(span, action, location)
        if not payload:
            return ""

        inferred_device, _, _ = self._infer_device_from_fragment(payload, action, parameter_kind)
        if inferred_device != device:
            return ""
        return payload

    def _has_device_anchor_evidence(
        self,
        span: str,
        command: Command,
        inferred_fragment: str = "",
    ) -> bool:
        """判断恢复候选是否具备设备锚点证据（显式或残片推断）。"""
        device = (command.device or "").strip()
        if not device:
            return False
        if device in span:
            return True
        if inferred_fragment and inferred_fragment in span:
            return True

        location = (command.location or "").strip()
        alias = self._extract_device_alias(span, location=location)
        if alias:
            canonical_alias = self._canonicalize_device(alias)
            if canonical_alias == device or alias == device:
                return True

        return False

    def _has_recovery_residue(
        self,
        span: str,
        command: Command,
        extra_consumed_tokens: Optional[List[str]] = None,
        ...
        for token in extra_consumed_tokens or []:
            ...
            if not token_text:
                continue
            token_pos = span.find(token_text)
            if token_pos < 0:
                continue
            positions.append((token_pos, token_pos + len(token_text)))
