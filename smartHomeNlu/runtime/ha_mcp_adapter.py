...
TOOL_NAME_ALIAS = {
    ...
    "ha_create_automation": "ha_config_set_automation",
    "ha_remove_automation": "ha_config_remove_automation",
}
...
TOOL_PROXY_HINT = {
    ...
    "ha_create_automation": "ha_call_write_tool",
    "ha_remove_automation": "ha_call_write_tool",
}
...
def _normalize_mcp_url(raw_url: str) -> str:
    ...
    # Accept bare host[:port][/path] input and normalize to HTTP URL.
...
def _flatten_exception_message(exc: BaseException, *, max_items: int = 4) -> str:
    messages: List[str] = []
    seen: set[int] = set()
    ...
    def _collect(err: BaseException) -> None:
        ...
        text = str(err).strip()
        item = text or err.__class__.__name__
    ...
    def tool_call(self, tool_name: str, params: Dict[str, Any] | None = None) -> Dict[str, Any]:
        ...
        if tool_name == "ha_create_automation":
            return {
                "success": True,
                "status_code": 200,
                "automation_id": "automation.runtime_generated",
            }
    ...
    def _invoke_remote_tool(self, tool_name: str, params: Dict[str, Any]) -> Dict[str, Any]:
        if self._remote_tool_runner is not None:
            try:
                ...
                error_text = _flatten_exception_message(exc)
                self._logger.warning("invoke_remote_tool exception name=%s error=%s", tool_name, error_text)
                return {
                    "success": False,
                    "error_code": "UPSTREAM_ERROR",
                    "status_code": 502,
    ...
    def _normalize_remote_result(self, tool_name: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if tool_name == "ha_create_automation":
            return {
                "success": True,
                "status_code": 200,
                "automation_id": merged.get("entity_id") or merged.get("automation_id"),
            }
